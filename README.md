...
| <img src="https://www.google.com/s2/favicons?domain=koreainvestment.com&sz=64" width="28"/> | `kis` | [한국투자증권](https://apiportal.koreainvestment.com/) | KRX 국내주식 | 1d | ✅ 체결가 | ✅ | ✅ 호스트·TR 자동 전환 | ✅ 검증 (모의) |
| <img src="https://www.google.com/s2/favicons?domain=kiwoom.com&sz=64" width="28"/> | `kiwoom` | [키움증권](https://openapi.kiwoom.com/) | KRX 국내주식 | 1d | ✅ 체결가 | ✅ | ✅ 호스트 자동 전환 | ✅ 검증 (모의) |
...
실시간 = 웹소켓 체결가 스트림(`StreamChannel.TRADES`). `✅ 체결가` 는 모의 웹소켓 장중 실측 통과(2026-09), `❌` 는 미구현 또는 브로커 스펙에 없음. 스트림이 없는 브로커에서는 폴링만 동작합니다 ([동작 방식](#동작-방식)).
...
| [Python](python/) | `python/` | 0.8.0 | 0개 (stdlib, 3.10+) · 실시간은 `websockets` 선택 설치 | `pip install ./python` (실시간: `pip install './python[stream]'`) |
| [JavaScript/TypeScript](js/) | `js/` | 0.8.0 | decimal.js (Node 22+, 실시간은 내장 WebSocket) | `npm install ./js` |
