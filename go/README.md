...
버전 **0.10.0** (태그 `go/v0.10.0`) · Go **1.26+** (`go.mod` 기준) · 의존성 `github.com/shopspring/decimal`, `github.com/coder/websocket`(실시간 스트림) · MIT
...
```bash
go get github.com/tauthdev/hermetix-trading-core/go@v0.10.0
```
...
모노레포의 `go/` 하위 모듈이라 태그가 `go/v0.10.0` 형식입니다. 패키지 이름은 `hermetix` 로 별칭을 두는 것을 권장합니다.
