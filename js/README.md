...
버전 0.10.0 · Node 22+ · 런타임 의존성은 `decimal.js` 하나(실시간 웹소켓은 Node 22 내장 `WebSocket`) · MIT
...
- ✅ 검증 = 실서버 스모크(시세→캔들→계좌→주문)를 통과한 환경. ⚠️ 미검증 = 공식 SDK·문서에서 역추적한 구현으로, 네 언어 컨포먼스 테스트는 통과했지만 실측 전이라 스펙 해석 오류가 있을 수 있습니다. 상태는 [루트 README 지원 브로커 표](../README.md#지원-브로커)와 같습니다
