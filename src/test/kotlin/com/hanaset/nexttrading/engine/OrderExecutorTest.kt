...
import com.hanaset.nexttrading.client.dto.OrderSide
...
class OrderExecutorTest {
    ...
    private val bracketMonitor = BracketMonitor(nextApiClient)
    private val tradingGuard = mockk<TradingGuard>(relaxed = true)
    private val executor = OrderExecutor(nextApiClient, bracketMonitor, tradingGuard)

    @Test
    fun `매도는 보유 수량으로 클램프된다`() {
        every { tradingGuard.isHalted } returns false
        val captured = slot<CreateOrderRequest>()
        every { nextApiClient.createOrder(capture(captured)) } returns OrderResponse(orderId = "ord_1", status = OrderStatus.SUBMITTED)
    ...
    fun `보유가 없으면 매도를 스킵한다`() {
        ...
        verify(exactly = 0) { nextApiClient.createOrder(any()) }
    }
    ...
    fun `익절, 손절가가 있는 매수는 브라켓으로 등록된다`() {
        every { tradingGuard.isHalted } returns false
        every { nextApiClient.createOrder(any()) } returns OrderResponse(orderId = "ord_9", status = OrderStatus.SUBMITTED)
