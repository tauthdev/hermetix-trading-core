...
class KiwoomApiClient(
    ...
    override fun cancelOrder(orderId: String): OrderResponse {
        val order = openOrders().firstOrNull { it.orderNo() == orderId }
            ?: throw BrokerApiException(404, "order-not-found", "미체결 주문을 찾을 수 없습니다: $orderId")
    ...
    private fun call(path: String, apiId: String, body: Map<String, String>): JsonNode {
        ...
        while (true) {
            try {
                ...
                if (e.message?.contains("요청 개수를 초과") == true && attempt < 3) {
                    attempt++
        ...
        return restClient.post()
            ...
            .exchange { _, res ->
                ...
                if (!res.statusCode.is2xxSuccessful || node.path("return_code").asInt(-1) != 0) {
                    throw BrokerApiException(
                        res.statusCode.value(),
                        node.path("return_code").asText(null),
                        "키움($apiId) ${node.path("return_msg").asText("")}".trim(),
                    )
                }
                node
            }!!
        ...
        val node = restClient.post()
            ...
            .exchange { _, res ->
                ...
                if (!res.statusCode.is2xxSuccessful || n.path("return_code").asInt(-1) != 0 || !n.hasNonNull("token")) {
                    throw BrokerApiException(res.statusCode.value(), n.path("return_code").asText(null), "키움 토큰 발급 실패: ${n.path("return_msg").asText("")}")
                }
                n
            }!!
