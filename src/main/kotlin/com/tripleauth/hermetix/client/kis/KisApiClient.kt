...
class KisApiClient(
    ...
    override fun getOrder(orderId: String): OrderResponse =
        dailyOrders().firstOrNull { it.path("odno").asText() == orderId }?.toOrderResponse()
            ?: throw BrokerApiException(404, "order-not-found", "주문을 찾을 수 없습니다: $orderId")
    ...
    private fun call(
        ...
        while (true) {
            try {
                ...
                if (e.errorCode == "EGW00201" && attempt < 3) {
                    attempt++
            ...
            .exchange { _, res ->
                ...
                if (!res.statusCode.is2xxSuccessful || node.path("rt_cd").asText("") != "0") {
                    throw BrokerApiException(
                        ...
                        node.path("msg_cd").asText(null),
                        "KIS(${trId}) ${node.path("msg1").asText("")}".trim(),
                    )
                }
                node
            }!!
        ...
        val node = restClient.post()
            ...
            .exchange { _, res ->
                ...
                if (!res.statusCode.is2xxSuccessful || !n.hasNonNull("access_token")) {
                    throw BrokerApiException(res.statusCode.value(), n.path("error_code").asText(null), "KIS 토큰 발급 실패: ${n.path("error_description").asText("")}")
                }
                n
            }!!
