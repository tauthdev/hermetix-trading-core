...
- `POST /v1/usage` — 서명 검증(`401`) → 형식 검증(`400`, 256KB 초과 `413`) → 타당성 검사(위반은 `202` 후 폐기) → `usage_bucket` 에 upsert (같은 키면 건수 합산, 지연 분포는 count 가중 평균으로 병합). 응답 `202` 빈 본문
...
- 랭킹은 요청마다 계산하지 않는다 — 시간 버킷 원본을 **일 단위로 롤업**하고, **매시 스냅샷**(7d·30d)을 계산해 저장하며 API 는 최신 스냅샷을 돌려준다 (최대 1시간 지연, 응답의 `computedAt`). 시간 버킷 원본은 90일 보존, 일 롤업은 영구
